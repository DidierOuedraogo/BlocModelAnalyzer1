...
with st.sidebar:
    st.markdown('<h2 style="color:white;">Chargement des données</h2>', unsafe_allow_html=True)
    ...
    st.markdown('<h3 style="color:white;">Fichiers DXF (optionnels)</h3>', unsafe_allow_html=True)
    ...
    <div style="color:white;font-size:0.9rem;">
        <b>BlockModelAnalyzer v1.0.0</b><br>
        ...
        if not filtered_df.empty:
            ...
            with col1:
                ...
                if st.button("Exporter les statistiques (CSV)"):
                    ...
                    href = f'<a href="data:file/csv;base64,{b64}" download="stats_{grade_column}_{datetime.now().strftime("%Y%m%d")}.csv" class="download-button">Télécharger CSV</a>'
        ...
        if tonnage_column in filtered_df.columns and not filtered_df.empty:
            ...
            with col1:
                if st.button("Exporter en CSV"):
                    ...
                    b64 = base64.b64encode(csv.encode()).decode()
                    href = f'<a href="data:file/csv;base64,{b64}" download="tonnage_teneur_{datetime.now().strftime("%Y%m%d")}.csv" class="download-button">Télécharger CSV</a>'
                    st.markdown(href, unsafe_allow_html=True)
            ...
            with col2:
                if st.button("Exporter en Excel"):
                    ...
                    href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="tonnage_teneur_{datetime.now().strftime("%Y%m%d")}.xlsx" class="download-button">Télécharger Excel</a>'
                    st.markdown(href, unsafe_allow_html=True)
        else:
...
# Footer avec date et copyright
st.markdown("""
<div class="footer">
    <p>
        BlockModelAnalyzer v1.0.0<br>
        © 2025 Didier Ouedraogo, P.Geo. Tous droits réservés.<br>
        Généré le {0}
    </p>
</div>
